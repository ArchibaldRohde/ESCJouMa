...
raw_samples = []
with open("DBPSK_hierdie_een.txt") as fileobject:
    l = 0
...
clock = 0.5*np.cos(fsy*2*np.pi*t + 54*np.pi/40) + .5 #vir DBPSK
...
for j in range(len(phase_ons_is_klaar_gesamples)-1):
    if j < 900:
        ...
        qsam.append(qsam[j]+m)
        qdiff[j] = (qsam[j]-phase_ons_is_klaar_gesamples[j])
