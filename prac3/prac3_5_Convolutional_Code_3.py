...
stront = ""
bitstr = ''
s = s[10:-1] #speel met dit as niks werk nie
print(s)
...
for i in range(math.floor((len(s)/5)-3)):
    code = s[0:5]
    b = BitArray(bin=code)
    s = s[5:]
    nextcode = s[0:5]
    nextb = BitArray(bin=nextcode)
    nextnextcode = s[5:10]
    nextnextb = BitArray(bin=nextnextcode)
    nextnextnextcode = s[10:15]
    nextnextnextb = BitArray(bin=nextnextnextcode)
    ...
    ###############################################################^Die actual state >die next state
        #een in die toekoms
        a02 = np.count_nonzero(nextb ^ BitArray(bin=Data[0]))
        a12 = np.count_nonzero(nextb ^ BitArray(bin=Data[1]))
        a22 = np.count_nonzero(nextb ^ BitArray(bin=Data[2]))
        a32 = np.count_nonzero(nextb ^ BitArray(bin=Data[3]))

        #################################################################
        #twee in die toekoms
        a020 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[0]))
        a120 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[1]))
        a220 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[2]))
        a320 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[3]))

            ###########################################################
        #drie in die toekoms
        a0200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[0]))
        a1200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[1]))
        a2200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[2]))
        a3200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[3]))

        coolcoolnnn = [a0200, a1200, a2200, a3200]
        axxx0 = min(coolcoolnnn)

        a0200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[4]))
        a1200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[5]))
        a2200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[6]))
        a3200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[7]))

        coolcoolnnn = [a0200, a1200, a2200, a3200]
        axxx1 = min(coolcoolnnn)
        ...
        coolcoolnnn = [a0200, a1200, a2200, a3200]
        axxx3 = min(coolcoolnnn)
            ###########################################################

        coolcoolnn = [a020+axxx0,a120+axxx1,a220+axxx2,a320+axxx3]
        axx0 = min(coolcoolnn)

        a020 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[4]))
        a120 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[5]))
        a220 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[6]))
        a320 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[7]))
        ...
        axx3 = min(coolcoolnn)
        ##########################################################
        coolcooln = [a02+axx0, a12+axx1, a22+axx2, a32+axx3]
        ax0 = min(coolcooln)


        a02 = np.count_nonzero(nextb ^ BitArray(bin=Data[4]))
        a12 = np.count_nonzero(nextb ^ BitArray(bin=Data[5]))
        a22 = np.count_nonzero(nextb ^ BitArray(bin=Data[6]))
        a32 = np.count_nonzero(nextb ^ BitArray(bin=Data[7]))
        #################################################################
        a020 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[0]))
        a120 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[1]))
        a220 = np.count_nonzero(nextnextb ^ BitArray(bin=Data[2]))
        ...
        a020 = np.count_nonzero(nextb ^ BitArray(bin=Data[4]))
        a120 = np.count_nonzero(nextb ^ BitArray(bin=Data[5]))
        a220 = np.count_nonzero(nextb ^ BitArray(bin=Data[6]))
        a320 = np.count_nonzero(nextb ^ BitArray(bin=Data[7]))
        ###########################################################
        a0200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[0]))
        a1200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[1]))
        a2200 = np.count_nonzero(nextnextnextb ^ BitArray(bin=Data[2]))
...
#Data = ["00000", "10110", "11101", "01011", "11010", "01100", "00111", "10001"]
        ...
        a01 = np.count_nonzero(b ^ BitArray(bin=Data[4]))
        a11 = np.count_nonzero(b ^ BitArray(bin=Data[5]))
        a21 = np.count_nonzero(b ^ BitArray(bin=Data[6]))
        a31 = np.count_nonzero(b ^ BitArray(bin=Data[7]))
    ###############################################################^Die actual state >die next state
        #een in die toekoms
        a02 = np.count_nonzero(nextb ^ BitArray(bin=Data[0]))
        a12 = np.count_nonzero(nextb ^ BitArray(bin=Data[1]))
