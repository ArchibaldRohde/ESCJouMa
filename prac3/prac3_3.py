...
clock2 = 0.5*np.cos(samplefreq*2*np.pi*time + 5*np.pi/5) + .5 #vir DBPSK


no_zerosH = []
ons_sample_hier_jaH = np.zeros([300000], dtype=complex)
flaggie = 0  ##################################################################Die issue is dalk hier
for i in range(len(ons_sample_hier_jaH)):
    if (clock2[i] >= 0.985) and (ons_sample_hier_jaH[i - 1] == 0) and (ons_sample_hier_jaH[i - 2] == 0) and (
            ons_sample_hier_jaH[i - 3] == 0):
        ons_sample_hier_jaH[i] = h_t[i]
    if not (ons_sample_hier_jaH[i] == 0.0):
        no_zerosH.append(h_t[i])
...
plt.plot(ons_sample_hier_jaH,  label='no_zerosH')
plt.plot(clock2, label='clock')
