...
t1 = np.arange(0, 15000 / samplefreq, 1 / samplefreq)
clock = 0.5*np.cos(fsy*2*np.pi*t + 38*np.pi/20) + .5 #vir DBPSK
h_t = (np.sin((np.pi*(t1)/Ts)*(1-beta)) + (4*beta*(t1)/Ts)*(np.cos((np.pi*(t1)/Ts)*(1 + beta))))/(np.pi*(t1)*(1-(4*beta*(t1)/Ts)**2))
#h_t[Ts/2] = (beta/(Ts*np.sqrt(2)))*((1+2/np.pi)*np.sin(np.pi/(4*beta))+(1-2/np.pi)*np.cos(np.pi/(4*beta)))
h_t[0] = (1/Ts)*(1+beta*((4/np.pi)-1))
h_t2 = np.flip(h_t)
h_t3 = np.concatenate((h_t, h_t2),axis=0)


jaaaa = signal.fftconvolve(raw_samples, h_t3, mode='full')
jaaaa2 = jaaaa[0:30000]
jaaaa2 = jaaaa2/np.max(jaaaa2)
...
for i in range(len(ons_sample_hier_ja)):
    if (clock[i] >= 0.985) and (ons_sample_hier_ja[i - 1] == 0) and (ons_sample_hier_ja[i - 2] == 0) and (
        ...
        ons_sample_hier_ja[i] = jaaaa2[i]
        if not ((abs(jaaaa2[i]) < 0.05) and (abs(jaaaa2[i]) > -0.05)):
            flaggie = 1
        if flaggie:
            no_zeros.append(jaaaa2[i])

t1 = np.arange(0, len(no_zeros) / samplefreq, 1 / samplefreq)


plt.figure()
#plt.plot(jaaaa2, label='filter')
plt.plot(np.real(raw_samples), label='raw_samples real')
plt.plot(np.real(jaaaa2), label='raw_samples imag')
