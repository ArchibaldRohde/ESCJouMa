...
'''
#fsy = 98684 #vir 16QAM
fsy = 93744#102728#93728 #vir ASK
#fsy = 97396#102732 #vir BPSK
#fsy = 107135 #vir D8PSK 94929
...
clock = 0.5*np.cos(fsy*2*np.pi*t + 25*np.pi/40) + .5 #vir ASK    np.pi-0.6
#clock = 0.5*np.cos(fsy*2*np.pi*t+np.pi*119/80) + .5 #vir BPSK
#clock = 0.5*np.cos(fsy*2*np.pi*t+28*np.pi/20) + .5 #vir D8PSK+0.2
...
# ################################################################4-ASK
b = np.abs(mod_out_hier_sample)
a = np.zeros(len(mod_out_hier_sample), dtype=complex)
powow = np.zeros(len(mod_out_hier_sample), dtype=complex)

bangle = 0
for i in range(len(mod_out_hier_sample)):
    if bangle == 0:
        if ((phi[i]) > -1) and (phi[i] < 1):
            bangle = 0
        else:
            bangle = np.pi
    elif bangle == np.pi:
        if ((phi[i]) > -1) and (phi[i] < 1):
            ...
            bangle = 0
    print(bangle)
    powow[i] = (b[i]*np.exp(1j*bangle))
    a[i] = np.real(b[i]*np.exp(1j*bangle))
...
plt.plot(np.real(powow), np.imag(powow), 'go', label='Sync IQ')
plt.legend()
plt.yscale('linear')
